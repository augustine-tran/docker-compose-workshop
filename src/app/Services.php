...
use Phalcon\Session\Adapter\Files as FilesSessionAdapter;
...
class Services extends \Base\Services
{
    ...
    {
        $config = $this->get('config')->get('session')->toArray();

        $session = null;
        if ($config['type'] === 'redis') {
            $redisConfig = $this->get('config')->get('redis')->toArray();
            $session = new RedisSessionAdapter($redisConfig);
        } else {
            $session = new FilesSessionAdapter();
        }
